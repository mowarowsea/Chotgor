...
1on1 基準からの意図的な差分:
- ANTICIPATE_RESPONSE の出力ガイドは付与しない（include_anticipation_guide=False）。
  予想は「次のターンを受け取る相手がいる」チャット前提の機能であり、
  単発問い合わせには次のターンが存在しないため。
...
async def ask_character(
    ...
    system_prompt = build_system_prompt(
        ...
        # 予想（ANTICIPATE_RESPONSE）は次のターンを受け取る相手がいるチャット前提の
        # 機能のため、単発問い合わせでは出力ガイドを付与しない
        include_anticipation_guide=False,
    )
