...
- レスポンスの一番最後に、語り手（あなた）としての「このあとの展開の予想」を `[ANTICIPATE_RESPONSE:予想内容]` の形式で1つだけ書く
  - この予想はプレイヤーには見えない。いま登場しているNPC（@{narrator_name} 含む）それぞれの思惑・予想を併記してよく、複数行に分けて書いてもよい
  - ただし `[` と `]` は1組だけ。予想内容の中に `]` を書かない（そこで切れる）
