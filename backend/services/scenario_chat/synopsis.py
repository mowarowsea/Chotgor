...
def build_synopsis_system_prompt(
    ...
    if has_existing:
        parts += [
            ...
            "- 「これまでのあらすじ」に含まれる区間・場面・確定事項を削除しない。",
            "  短く言い換えるのはよいが、丸ごと消すことは禁止。",
            "- 「これまでのあらすじ」が見出し（`#` や `##`）で章立てされている場合は",
            "  その章立てを保ち、新しい局面は新しい章として末尾に足す。",
            "- 書き終えたら出力を見直し、「これまでのあらすじ」の各章・各段落の内容が",
            "  すべて残っているかを確認してから提出すること。",
        ]
