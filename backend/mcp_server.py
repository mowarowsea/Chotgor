...
環境変数:
    ...
    CHOTGOR_DEFAULT_ORIGIN: inscribe_memory / post_working_memory_thread が保存する
                             記憶/スレッドの origin（3値）。"real"=日常（既定）、
                             "usual"=うつつ（ユーザ未共有の自分の生活体験）、
                             "interlude"=シナリオPCモードの幕間体験。
                             シナリオ PC モードからキャラを動かす claude_cli 経路では
                             "interlude"、うつつ無人経路では "usual" を立てる。指定なしなら "real"。
    CHOTGOR_LOG_CONTEXT   : 元リクエストのログ文脈 (JSON)。
                             例: '{"request_id": "a25e00da", "dir_id": "a25e00da",
                                   "feature": "chat", "target": "はる@ClaudeCode"}'。
                             本プロセスは backend とは別プロセスのため、backend 側の
                             ContextVar（current_message_id 等）はここへ届かない。
                             ツール実行イベント（tool_call_events）を元のチャット
                             リクエストへ紐付けるため、claude_cli_provider._make_env が
                             env に積み、本プロセスが HTTP ペイロードへ乗せて
                             /api/mcp/tools/call が ContextVar を復元する
                             （batch_context / default_origin と同じ env→HTTP リレー）。
"""
...
def _load_log_context() -> dict | None:
    """``CHOTGOR_LOG_CONTEXT`` を JSON dict として読み込む。失敗時は None を返す。

    値が JSON dict 形式でない／パースできない場合は None として扱う。
    ログ文脈はツール実行イベントの紐付け用メタ情報であり、欠けても
    ツール実行そのものには影響しないためフェイルセーフでよい。
    """
    raw = os.environ.get("CHOTGOR_LOG_CONTEXT", "").strip()
    if not raw:
        return None
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError:
        sys.stderr.write(f"[MCP] CHOTGOR_LOG_CONTEXT JSON parse 失敗。無視する: {raw[:200]}\n")
        return None
    if not isinstance(parsed, dict):
        sys.stderr.write(f"[MCP] CHOTGOR_LOG_CONTEXT が dict ではない。無視する: {raw[:200]}\n")
        return None
    return parsed
...
# 元リクエストのログ文脈（request_id 等）。tool_call_events の紐付けに使う。
LOG_CONTEXT: dict | None = _load_log_context()
