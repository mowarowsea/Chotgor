...
class DriftManager:
    ...
    def add_drift(self, session_id: str, character_id: str, content: str) -> object:
        ...
        """
        result = self.sqlite.add_session_drift(session_id, character_id, content)
        logger.info(
            "drift 追加 char=%s session=%s content=%.50s",
            character_id, session_id, content,
        )
        return result
    ...
    def reset_drifts(self, session_id: str, character_id: str) -> int:
        ...
        Returns:
            削除件数。
        """
        deleted = self.sqlite.reset_session_drifts(session_id, character_id)
        logger.info("drift リセット char=%s session=%s deleted=%d", character_id, session_id, deleted)
        return deleted
