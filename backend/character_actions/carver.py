...
class Carver:
    ...
    def carve_narrative_from_text(self, text: str) -> str:
        ...
        for m in matches["CARVE_NARRATIVE"]:
            ...
            if not content:
                continue
            # 引数はツール呼び出し方式（CARVE_NARRATIVE_SCHEMA）と同じキー名で記録し、
            # Logs 画面の表示変換（tool_tags.tool_call_to_structured_tag）を共用する。
            event_args = {"mode": mode, "content": content}
            try:
                self.carve_narrative(mode, content)
                record_tool_event("carve_narrative", event_args, source="tag")
            except Exception as e:
                logger.exception("更新失敗 char=%s", self.character_id)
                record_tool_event(
                    "carve_narrative", event_args,
