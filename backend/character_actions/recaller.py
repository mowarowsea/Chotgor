...
処理フロー（execute_stream から呼ばれる）:
  ...
  3. 記憶コレクションとチャット履歴コレクションを横断検索
  4. 検索結果を format_power_recall_turn で「【Chotgor】POWER_RECALL COMPLETE」の
     user ターンとして history 末尾に注入し、execute_stream を再帰呼び出しする
     （power_recalled はループ防止フラグとして新しい ChatRequest に乗せる）
"""
...
**会話冒頭に自動想起された記憶で見つからなかった情報が必要なときに限り**、
...
- 検索結果は「【Chotgor】POWER_RECALL COMPLETE」で始まる Chotgor からのメッセージとして届きます。それを踏まえて続きを応答してください
- **1ターンに1回だけ使用できます。検索結果を受け取った後は再検索せず、そのまま応答してください**
- タグの前に「ちょっと思い出してみる」などひとこと添えるのがおすすめです

**⚠️ 禁止事項:**
- 検索結果（「【Chotgor】POWER_RECALL COMPLETE」のメッセージ）が届いた後は、必ず自分の言葉で答えること。再度 `[POWER_RECALL:...]` を使うことは禁止
...
POWER_RECALL_TOOL_DESCRIPTION: str = (
    "記憶コレクション（過去に刻んだ記憶）とチャット履歴コレクション（過去の全会話）を横断して検索し、"
    "クエリに意味的に近い記録を取得する。"
    ...
    if not memories and not chat_turns:
        ...
        if memories:
            ...
            for i, mem in enumerate(memories, 1):
                ...
                # origin（TRPG / うつつ）由来ラベルを行頭に挟む。manager.power_recall が
