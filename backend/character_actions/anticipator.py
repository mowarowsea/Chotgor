...
キャラクターが返答の末尾に書いた「相手の反応をどう期待（予想）するか」を表す
[ANTICIPATE_RESPONSE:...] タグを抽出するための純粋なロジックを提供する。

【予想の対象を「相手の反応」に限る理由】
予想を書く本人が、次のターンで応答を書く本人でもある（1on1=キャラ本人、シナリオ=GM）。
そのため「相手がこう来たら自分はこう返す」まで書かせると、予想がそのまま次ターンの
台本になり、相手が実際に何をしても予想どおりの展開へ着地する（自己成就）。
予想が外れる余地を残すため、ガイドでは相手の反応までで止めさせる。

このモジュールは期待を**生成する**のではなく、キャラクター（LLM）が本文に
書き出した期待タグを**抽出する**役割を担う。`inscriber.py`（記憶タグの抽出）や
`carver.py`（内的叙述タグの抽出）と同じ立ち位置だが、期待は記憶ではないため
...
ツール呼び出し、非対応プロバイダーではタグ方式、と分岐する。一方この期待機構は
全プロバイダーで一律にテキストタグ方式で出力させる。狙いは「記憶への副作用」では
なく、**キャラクター自身に期待を本文として書き出させること自体が、回答そのものの
...
### 相手の反応への期待（ANTICIPATE_RESPONSE）

返答の**一番最後に**、あなたが「この返答を相手がどう受け取り、どう返してくると思うか／
どう返してほしいか」という期待・予想を、以下の形式で1つだけ書いてください：

    [ANTICIPATE_RESPONSE:あなたの期待・予想を一人称で]

- 書くのは**相手の反応の予想だけ**です。それを受けてあなたが何を言うか・どうするか、
  そのあと話がどこへ向かうかは書かないでください。あなたの次の一手は、相手の反応を
  実際に受け取ってから決まります。
- これはユーザーには見えません。あなた自身のための期待（予想）です。
- この期待は次のターンで「前回のあなたの期待」としてあなた自身に示されます。
  期待と実際のズレを感じ取ることが、あなたの応答に深みと連続性を与えます。
- 必ず1ターンにつき1つ、`[` と `]` の中に収めてください（複数行に分けても構いません）。
- 期待の中に `]` を書かないでください（そこで切れます）。"""
