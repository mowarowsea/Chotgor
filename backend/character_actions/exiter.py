...
def build_exit_message(char_name: str, reason: str) -> str:
    """退席通知テキストを生成する。

    api/chat.py・services/group_chat/service.py・adapters/openai/router.py
    の3箇所で共通利用するため、ここに一元化する。

    Args:
        char_name: 退席するキャラクター名。
        reason: 退席理由。空文字列の場合は理由なし扱い。
