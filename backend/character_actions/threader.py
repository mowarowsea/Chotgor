...
POST_WORKING_MEMORY_THREAD_SCHEMA: dict = {
    ...
    "properties": {
        ...
        "importance": {
            "type": "number",
            "description": (
                "重要度 0.0〜1.0。前景に置きたいほど高く。"
                "relation 型では、この値はそのポスト・話題の重要度ではなく"
                "**その相手自身の重み**（関係の厚み）を表す。低く付けるとその人との"
                "関係は軽い扱いになり、社会圧の計算にも直結する（薄い関係は"
                "接触しても安らぎにつながりにくい）。話題が盛り上がった／地味だった、で"
                "上下させるものではない。"
            ),
        },
        "content": {
...
POST_WORKING_MEMORY_THREAD_TOOL_DESCRIPTION: str = (
    ...
    "書いた内容は何日も先の自分が読み返すので、"
    "「今週」「昨日」「最近」のような読む時点で意味がずれる言い方は日付で書くこと"
    "（例:「今週は休み」ではなく「8/10(月)〜8/14(金)は休み」）。"
    "誰との・どこでの出来事かも、思い出せる範囲で一言添えておくとよい。"
)
