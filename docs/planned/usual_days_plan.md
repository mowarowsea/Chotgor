...
### 11.5 初版の事故 — 種は「温度」であって「出来事」ではない（同日追記）

投入初日、種「ムカつく×雰囲気」に対して GM（Gemini 3.1 Flash Lite）が
**見知らぬ初老の男が本人の部屋を訪ねてくる**場面を作った（表札を確認しながら
「神谷、という表札はこちらで合っているのかね」）。物語の発端としか読めない出来事で、
放置すれば回収困難な伏線になるところだった。

- シーン自体は 4 レスポンスで `[SCENE_CLOSE]` しており、停止機構は正常。
- 記憶（`inscribed_memories` / `working_memory_posts`）への染み出しは無し。
- 該当 4 ターンと `timeline_events` 5 件を削除して巻き戻した（ユーザ判断）。
  冪等キー `usual_days_entry_run_*` は**残した** — 消すと同じ枠が再発火して
  同じ時間帯をやり直すことになるため、「その枠は流れた」ことにする。

原因は初版の提示文に**規模の枠が無かった**こと。「2 語から連想される状況を作れ」と
しか言っていないので、GM は自由に事件を立ち上げられる。加えて抽象 2 語は
**軽量モデルほど「一番わかりやすい絵」に落ちる** —— 「ムカつく」を人物の情動ではなく
場の不穏さと読んだ瞬間、不審者が最短の答えになる。抽象種はモデルの繊細さに依存する
設計だった、というのが教訓（当初は逆に「GM が 2 語を説明的に処理して終わる」方を
懸念していたが、振れたのは反対側だった）。

対処: 提示文に「これは場面の**温度**の指定であって出来事の指示ではない」と明示し、
立ち上げてはいけないもの（事件・来訪者・トラブル・知らない人物・不穏な気配）を
具体名で列挙。「この人物の生活は、この場面のあとも昨日までと同じように続きます」で
地続き性を釘刺しする。**語彙プール自体は据え置き**（「報せ」「予感」も日常の範囲で
描けるはず、という判断。枠で押さえきれなければ後で削る）。
