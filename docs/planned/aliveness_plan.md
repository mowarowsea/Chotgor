...
| 体調圧 | 身体のリズム | 疲労成分＝イベント密度の減衰積分 ＋ リズム成分＝固有周期の波 | 疲労は静かな日の指数減衰で回復（τ=1.5 ≒ 1日で約半分）。リズムは自力で基線に戻る |
...
- 疲労成分の正規化は**固定の魔法定数を置かず**、キャラ自身の平常活動量から動的に導く:
  `NORM = HEADROOM(3) × 平常件数/日 × τ`。「平常の3倍の負荷が続くと疲労1.0」で、よく喋る子は
  基準が上がり同じ量では疲れない（**封筒から導く**）。平常は直近30日を calendar-day 除算で測り、
  データが乏しい間（7日未満）は既定45件/日（NORM≈200）でコールドスタート。回復は**夜バッチに
  依存させず**、静かな日の指数減衰に一本化（night.chronicle の件数を回復項に使わない — 夜バッチは
  失敗しうるうえ回復の本体になり得ないため）。
...
- 圧力のプロンプトへの渡し方: 解釈済みの言葉ではなく**生に近い淡白な一行**
  ...
  圧＝物理、WM＝意味、の分業。閾値ごとに表現プールを持ち乱数で選ぶ（同じ文言の
  固定化＝キャラの口癖化を避けるため）。きわめて低圧（好調）のときも一行報告し、
  中間域（ニュートラル）だけは何も言わない（沈黙も情報）。
