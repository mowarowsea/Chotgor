...
- **発話閾値は絶対値ではなく、キャラ自身の分布の分位点で決める**。`meter_snapshots` の
  過去60日から分位点を取り、**上位20%＝高圧の一行／下位20%＝好調の一行／中間60%＝沈黙**。
  物理量の定式化を変えても表現が偏らない（絶対値だと定式化を触るたびに「言いすぎ／言わなすぎ」の
  再調整が必要になる — 上記の不採用A・Bはどちらも絶対値閾値との噛み合わせで破綻した）。
  - **沈黙ガードは必須**: 分位点は分布がどれだけ平坦でも必ず上位20%を作るため、素のままだと
    **毎日どれかの圧が何かを言う**状態になる（例: 60日間ずっと 0.44〜0.46 に収まっていても、
    0.46 の日は「上位20%」として高圧の一行が出る）。分布の幅（IQR）が下限未満のときは
    全圧を沈黙させ、**中間域だけは何も言わない＝沈黙も情報**という原則を分位点方式でも保つ。
  - ウォームアップ（メーター履歴が60日に満たない間）は絶対値閾値へフォールバックする。
