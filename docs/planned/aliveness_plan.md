...
> **圧力3変数の再定式化を 2026-08-26 に決定（§4.1 / §4.2 / §5.2 改訂・実装は未着手）。**
...
- **発話閾値は絶対値ではなく、キャラ自身の分布の分位点で決める**。`meter_snapshots` の
  ...
  - **沈黙ガードは必須**: 分位点は分布がどれだけ平坦でも必ず上位10%を作るため、素のままだと
    **毎日どれかの圧が何かを言う**状態になる（例: 60日間ずっと 0.44〜0.46 に収まっていても、
    0.46 の日は「上位10%」として強い表現が出る）。分布の幅（IQR）が下限 **0.05** 未満の圧は
    沈黙させ、**中間域だけは何も言わない＝沈黙も情報**という原則を分位点方式でも保つ。
    判定は**圧ごと**に行う（体調は平坦でも社会圧は動いている、という状態があるため）。
...
- **θ_hard は絶対値のまま据え置く**（0.95）。ここを相対化すると「その子にとっての限界」に
  ...
  - **絶対値のまま意味を保てる根拠**: §4.1 の疲労成分（対数中心化）は値がそのまま
    「平常の何倍の負荷か」を表す。θ_hard=0.95 は **平常の約2.0倍の負荷**に相当し、
    はるの実測では年14日ほどの発火頻度になる（θ_base=0.75 相当なら平常の約1.5倍・年29日）。
