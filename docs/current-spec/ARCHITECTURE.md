...
**GM の PC 指名は「ターンの明け渡し」** — GM が行頭 `@<PC名>:`（PC枠名・PCキャラ本名・
ユーザPC名のいずれか＝`suppress_names`）を書いた時点で、`ScenarioChatParser` が
`yielded_to` に記録し、`EnsembleEngine` がプロバイダのストリームをその場で打ち切る
（受信済みチャンクまでは `raw_response` に積むので、意味を拾う側は無傷）。GM が
「ここは PC が喋る番」と判断して書いた以上、その後ろに GM が続けた地の文は
**まだ起きていない出来事**であり、握りつぶして PC へ渡す。`ScenarioRouter` は
`forced_next_pc` を raw の末尾メンション解析より**優先**して採用する。
- 打ち切り粒度はチャンク単位（`@<PC名>:` を含むチャンクの受信までは raw に積む）。
  行の途中で切る精密制御はしない。
- 譲渡先が `routing_pcs` に居ない場合（うつつの不在ユーザPC 等）は、打ち切ったうえで
  @ALL フォールバックへ倒す（不在の相手を待たない）。ユーザPC 指名は通常モードでは
  ループを終了してユーザ入力待ちにする。
- 旧挙動は「PC ブロックの中身だけ捨てて Narrator へ復帰」で、GM が指名後も場面を
  進め続けていた。さらに `@<PC名>:` の後ろに `@Narrator:`／NPC 名が続くと末尾メンション
  解析がそちらを拾い、**指名した PC ではなく @ALL ランダム抽選**へ落ちる誤ルーティングも
  起きていた（`_next_after_gm` の `kind not in {"pc","all"}` 経路）。
...
枝ナビ（◀ 2/3 ▶）で引き直した結果を選び直せる。
**引き直しがエラーで終わったときは、巻き戻した枝を本線へ戻す**（失敗した試行で
途中まで保存されたターンは、中身のない兄弟枝を残さないよう物理削除してから戻す）。
戻さないと分岐点に活性ターンが無くなり、枝ナビも出ないので元のレスポンスへ辿れない。
履歴を読む経路は
...
- 不在のユーザ（GM のユーザ言動捏造を塞ぐ）: うつつ＝ユーザがそばにいない時間。狙いは
  ...
  - pc_summary（名簿）からはユーザを **外す**（routing_pcs で構築）── 名簿に並べると GM が
    ...
    が出ても parser 段で破棄する最終バックストップとする。破棄と同時に GM のストリームも
    打ち切るが（上記「GM の PC 指名は『ターンの明け渡し』」）、不在のユーザへは譲れないので
    @ALL フォールバックへ倒す。
...
- 毎分: action（2時間格子＋ジッター評価）・usual_days（うつつシーン＋push再開）・
  ...
  speech_reservation（発話予約 speak_later の発火。availability 再評価→
  `spontaneous_initiative_daily_cap` を reach_out と共有→
