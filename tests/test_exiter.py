"""backend.character_actions.exiter モジュールのユニットテスト。
...
from backend.character_actions.exiter import (
    Exiter,
    END_SESSION_SCHEMA,
...
class TestToolExecutorEndSession:
    ...
    def test_end_session_in_anthropic_tools(self):
        ...
        from backend.character_actions.executor import ANTHROPIC_TOOLS
    ...
    def test_end_session_in_openai_tools(self):
        ...
        from backend.character_actions.executor import OPENAI_TOOLS
...
# ─── character_actions/exiter.py ヘルパー関数テスト ─────────────────────────

class TestChatApiHelpers:
    """character_actions/exiter.py の退席メッセージ生成ヘルパーを検証する。

    build_exit_message() は api/chat.py・services/group_chat/service.py・
    adapters/openai/router.py の3箇所から共通利用される。
    """
    ...
    def test_build_exit_message_with_reason(self):
        ...
        msg = build_exit_message("Alice", "疲れた")
        assert "Alice" in msg
        assert "疲れた" in msg
        assert "退席" in msg
